...
description: |
  ...
  Output lưu tại posts/[tên sản phẩm]/prompt-template-1.json.
...
từ `context/templates/product-post/final-template-1.json` và điền tất cả `{variable}` từ dữ liệu
...
File `posts/[tên sản phẩm]/prompt-template-1.json` hoàn chỉnh, với mọi `{variable}` đã được điền.
...
- Đọc `context/templates/product-post/final-template-1.json` để nắm cấu trúc đầy đủ
...
**Quy tắc `file_ref` trong `input_3`:** Chỉ ghi tên file (ví dụ: `"z7801767378020_abc.jpg"`), không ghi đường dẫn đầy đủ.

---

### Phase 4: Lưu file

1. Kiểm tra folder `posts/[tên sản phẩm]/` — tạo nếu chưa có
2. Hiển thị preview phần `thông_tin_sản_phẩm_đã_điền` để user review nhanh
3. **Chờ user confirm** trước khi lưu
4. Lưu file `posts/[tên sản phẩm]/prompt-template-1.json`
5. Copy file ảnh dùng làm `input_3` (file_ref) vào `posts/[tên sản phẩm]/` với tên giữ nguyên
...
6. **Chờ user confirm**
7. Tạo folder `posts/[tên]/` nếu cần → lưu `prompt-template-1.json`
8. Copy file ảnh `input_3` (file_ref) vào `posts/[tên]/` giữ nguyên tên file
9. Thông báo path đầy đủ
